...
public:
    ...
    struct boss_latosiusAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_UNERRING_SHEAR);    //Как только в инсте появится зона или арея - докинуть снятие в spell_area!
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_UNERRING_SHEAR:
                        if (me->getVictim())
                            DoCast(me->getVictim(), SPELL_UNERRING_SHEAR);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_1:
                        ...
                        break;
                    case EVENT_2:
                        DoCast(me, SPELL_LEGACY_RAVENCREST, true);
