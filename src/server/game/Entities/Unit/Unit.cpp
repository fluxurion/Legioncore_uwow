...
bool Unit::HasCrowdControlAuraType(AuraType type, uint32 excludeAura) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if ((!excludeAura || excludeAura != (*itr)->GetSpellInfo()->Id) && //Avoid self interrupt of channeled Crowd Control spells like Seduction
            ((*itr)->GetSpellInfo()->HasAttribute(SPELL_ATTR0_BREAKABLE_BY_DAMAGE) || (*itr)->GetSpellInfo()->AuraInterruptFlags & (AURA_INTERRUPT_FLAG_TAKE_DAMAGE)))
            return true;
    return false;
}
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->damage)
    {
        ...
        for (AuraEffectList::const_iterator dmgShieldItr = vDamageShieldsCopy.begin(); dmgShieldItr != vDamageShieldsCopy.end(); ++dmgShieldItr)
        {
            ...
            victim->CalcAbsorbResist(this, SpellSchoolMask(i_spellProto->Misc.SchoolMask), SPELL_DIRECT_DAMAGE, damage, &absorb, &resist, i_spellProto);
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    for (AuraEffectList::iterator itr = vSchoolAbsorbCopy.begin(); (itr != vSchoolAbsorbCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
        ...
        if (_amount >= 0 && currentAbsorb && !(absorbAurEff->GetSpellInfo()->HasAttribute(SPELL_ATTR6_NOT_LIMIT_ABSORB)))
        {
...
void Unit::CalcHealAbsorb(Unit* victim, const SpellInfo* healSpell, uint32 &healAmount, uint32 &absorb)
{
    ...
    DamageInfo dmgInfo = DamageInfo(this, victim, healAmount, healSpell, SpellSchoolMask(healSpell->Misc.SchoolMask), SPELL_DIRECT_DAMAGE, 0);
    ...
    for (AuraEffectList::const_iterator i = vHealAbsorb.begin(); i != vHealAbsorb.end() && RemainingHeal > 0; ++i)
    {
        ...
        if (!((*i)->GetMiscValue() & healSpell->Misc.SchoolMask))
            continue;
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    bool canDodge = true;
    bool canParry = true;
    bool canBlock = (spell->HasAttribute(SPELL_ATTR3_BLOCKABLE_SPELL)) != 0;
...
Aura* Unit::_TryStackingOrRefreshingExistingAura(SpellInfo const* newAura, uint32 effMask, Unit* caster, int32* baseAmount /*= NULL*/, Item* castItem /*= NULL*/, ObjectGuid casterGUID /*= 0*/)
{
    ...
    if (casterGUID.IsEmpty())
    {
        ...
        else if(!newAura->AuraOptions.CumulativeAura)
            casterGUID = caster->GetGUID();
    }
...
void Unit::_ApplyAura(AuraApplication * aurApp, uint32 effMask)
{
    ...
    {
        if (spellInfo->HasAttribute(SPELL_ATTR2_FOOD_BUFF))
        {
            for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
                if (aura->GetEffect(i))
...
void Unit::RemoveAurasWithAttribute(uint32 flags)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (spell && spell->Misc.Attributes[0] & flags)
            RemoveAura(iter);
        else
            ++iter;
    }
...
void Unit::RemoveArenaAuras()
{
    ...
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (!(aura->GetSpellInfo()->HasAttribute(SPELL_ATTR4_UNK21))  // don't remove stances, shadowform, pally/hunter auras
            ...
            || aura->GetSpellInfo()->HasAttribute(SPELL_ATTR2_UNK3))
            RemoveAura(iter);
        else
            ++iter;
    }
...
void Unit::RecalcArenaAuras()
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        {
            //recalc only if need
            if ((aura->GetSpellInfo()->HasAttribute(SPELL_ATTR8_NOT_IN_BG_OR_ARENA)) || (aura->GetSpellInfo()->HasAttribute(SPELL_ATTR4_NOT_USABLE_IN_ARENA_OR_RATED_BG)))
...
AuraEffect* Unit::GetAuraEffect(AuraType type, SpellFamilyNames name, uint32 iconId, uint8 effIndex) const
{
    ...
    for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (spell->Misc.SpellIconID == iconId && spell->ClassOptions.SpellClassSet == uint32(name) && !spell->ClassOptions.SpellClassMask)
            return *itr;
    }
    return NULL;
}
...
bool Unit::HasNegativeAuraWithAttribute(uint32 flag, ObjectGuid guid)
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (!iter->second->IsPositive() && aura->GetSpellInfo()->Misc.Attributes[0] & flag && (guid.IsEmpty() || aura->GetCasterGUID() == guid))
            return true;
    }
    return false;
}
...
bool Unit::HasAuraWithAttribute(uint32 Attributes, uint32 flag) const
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (aura->GetSpellInfo()->Misc.Attributes[Attributes] & flag)
            return true;
    }
    return false;
}
...
int32 Unit::GetTotalAuraModifier(AuraType auratype, bool raid) const
{
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(), next; i != mTotalAuraList.end(); i = next)
    {
        ...
        if (AuraEffect* eff = (*i))
        {
            if (raid && (eff->GetSpellInfo()->HasAttribute(SPELL_ATTR7_CONSOLIDATED_RAID_BUFF)))
            {
...
bool Unit::HandleDummyAuraProc(Unit* victim, DamageInfo* dmgInfoProc, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, double cooldown)
{
    ...
    switch (dummySpell->ClassOptions.SpellClassSet)
    {
        ...
        {
            // Magic Absorption
            if (dummySpell->Misc.SpellIconID == 459)             // only this spell has SpellIconID == 459 and dummy aura
            {
            ...
            // Master of Elements
            if (dummySpell->Misc.SpellIconID == 1920)
            {
                if (!procSpell)
                    return false;
            ...
            // Arcane Potency
            if (dummySpell->Misc.SpellIconID == 2120)
            {
                if (!procSpell)
                    return false;
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch (dummySpell->Id)
            {
                ...
                case 56636:
                {
                    ...
                    if (aura)
                    {
                        if(stack > aura->GetSpellInfo()->AuraOptions.CumulativeAura)
                            stack = aura->GetSpellInfo()->AuraOptions.CumulativeAura;
            ...
            // Retaliation
            if (dummySpell->ClassOptions.SpellClassMask[1] & 0x8)
            {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummySpell->ClassOptions.SpellClassMask.IsEqual(0, 0, 0) && dummySpell->Misc.SpellIconID == 1932)
            {
            ...
            // Fel Synergy
            if (dummySpell->Misc.SpellIconID == 3222)
            {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                {
                    // Shadow Word: Pain
                    if (procSpell->ClassOptions.SpellClassMask[0] & 0x8000)
                        triggered_spell_id = 40441;
                    // Renew
                    else if (procSpell->ClassOptions.SpellClassMask[0] & 0x40)
                ...
                case 70770:
                    // Flash Heal
                    if (procSpell->ClassOptions.SpellClassMask[0] & 0x800)
                    {
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (dummySpell->Id)
            {
                ...
                case 54832:
                {
                    if (procSpell->Misc.SpellIconID != 62)
                        return false;
                ...
                case 40442:
                {
                    ...
                    // Starfire
                    if (procSpell->ClassOptions.SpellClassMask[0] & 0x4)
                    {
                        triggered_spell_id = 40445;
                        chance = 25.0f;
                    }
                    // Rejuvenation
                    else if (procSpell->ClassOptions.SpellClassMask[0] & 0x10)
                    {
                        triggered_spell_id = 40446;
                        chance = 25.0f;
                    }
                    // Mangle (Bear) and Mangle (Cat)
                    else if (procSpell->ClassOptions.SpellClassMask[1] & 0x00000440)
                    {
                ...
                {
                    // Wrath & Starfire
                    if ((procSpell->ClassOptions.SpellClassMask[0] & 0x5) && (procEx & PROC_EX_CRITICAL_HIT))
                    {
            ...
            // Living Seed
            if (dummySpell->Misc.SpellIconID == 2860)
            {
            ...
            // King of the Jungle
            else if (dummySpell->Misc.SpellIconID == 2850)
            {
                // Effect 0 - mod damage while having Enrage
                if (effIndex == 0)
                {
                    if (!procSpell || !(procSpell->ClassOptions.SpellClassMask[0] & 0x00080000) || procSpell->Misc.SpellIconID != 961)
                        return false;
                ...
                {
                    if (!procSpell || !(procSpell->ClassOptions.SpellClassMask[2] & 0x00000800) || procSpell->Misc.SpellIconID != 1181)
                        return false;
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            switch (dummySpell->Misc.SpellIconID)
            {
        ...
        case SPELLFAMILY_HUNTER:
        {
            switch (dummySpell->Misc.SpellIconID)
            {
                case 3560: // Rapid Recuperation
                {
                    ...
                    if (!(procSpell->ClassOptions.SpellClassMask[1] & 0x01000000))
                        return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->Misc.SpellIconID == 3017)
            {
                target = this;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 40470:
                {
                    ...
                    if (procSpell->ClassOptions.SpellClassMask[0] & 0xC0000000)
                    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 40463:
                {
                    ...
                    float chance;
                    if (procSpell->ClassOptions.SpellClassMask[0] & 0x1)
                    {
                        triggered_spell_id = 40465;         // Lightning Bolt
                        chance = 15.0f;
                    }
                    else if (procSpell->ClassOptions.SpellClassMask[0] & 0x80)
                    {
                        triggered_spell_id = 40465;         // Lesser Healing Wave
                        chance = 10.0f;
                    }
                    else if (procSpell->ClassOptions.SpellClassMask[1] & 0x00000010)
                    {
                ...
                {
                    // Lava Burst
                    if (procSpell->ClassOptions.SpellClassMask[1] & 0x1000)
                    {
                ...
                {
                    // Chain Heal
                    if ((procSpell->ClassOptions.SpellClassMask[0] & 0x100) && (procEx & PROC_EX_CRITICAL_HIT))
                    {
                ...
                case 70811:
                {
                    ...
                    if (procSpell->ClassOptions.SpellClassMask[0] & 0x3)
                    {
            ...
            // Frozen Power
            if (dummySpell->Misc.SpellIconID == 3780)
            {
                if (!target)
                    return false;
        ...
        {
            // Blood-Caked Blade
            if (dummySpell->Misc.SpellIconID == 138)
            {
            ...
            // Butchery
            if (dummySpell->Misc.SpellIconID == 2664)
            {
                basepoints0 = triggerAmount;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, DamageInfo* dmgInfoProc, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, double cooldown)
{
    ...
    if (sSpellMgr->GetSpellInfo(trigger_spell_id) == NULL)
    {
        switch (auraSpellInfo->ClassOptions.SpellClassSet)
        {
            ...
            case SPELLFAMILY_HUNTER:
            {
                if (auraSpellInfo->Misc.SpellIconID == 3247)     // Piercing Shots
                {
    ...
    switch (trigger_spell_id)
    {
        ...
        case 53817:
        {
            ...
            if (AuraEffect const* aurEff = GetAuraEffect(70832, 0))
                if (Aura const* maelstrom = GetAura(53817))
                    if ((maelstrom->GetStackAmount() == maelstrom->GetSpellInfo()->AuraOptions.CumulativeAura - 1) && roll_chance_i(aurEff->GetAmount()))
        ...
        case 58679:
        {
            // Proc only from healing part of Death Coil. Check is essential as all Death Coil spells have 0x2000 mask in ClassOptions.SpellClassMask
            if (!procSpell || !(procSpell->ClassOptions.SpellClassSet == SPELLFAMILY_DEATHKNIGHT && procSpell->ClassOptions.SpellClassMask[0] == 0x80002000))
                return false;
            break;
        }
        // Glyph of Death Grip
        ...
        case 70893:
        {
            ...
            if (!(procSpell->ClassOptions.SpellClassMask[1] & 0x10000000) && (procEx != PROC_EX_CRITICAL_HIT))
                return false;
            // check if we're procced by Claw, Bite or Smack (need to use the spell icon ID to detect it)
            if (!(procSpell->Misc.SpellIconID == 262 || procSpell->Misc.SpellIconID == 1680 || procSpell->Misc.SpellIconID == 473))
                return false;
            break;
        }
        // Shadow's Fate (Shadowmourne questline)
...
Unit* Unit::GetMagicHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    if (spellInfo->HasAttribute(SPELL_ATTR0_ABILITY) || spellInfo->HasAttribute(SPELL_ATTR1_CANT_BE_REDIRECTED) || spellInfo->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) || spellInfo->HasAttribute(SPELL_ATTR4_UNK15))
        return victim;
...

    {
            ...
            switch ((*i)->GetMiscValue())
            {
                ...
                case 4993:
                {
                    ...
                    for (AuraApplicationMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                    {
                        Aura const* aura = itr->second->GetBase();
                        SpellInfo const* spell = aura->GetSpellInfo();
                        if (spell->ClassOptions.SpellClassSet != SPELLFAMILY_WARLOCK || !(spell->ClassOptions.SpellClassMask[1] & 0x0004071B || spell->ClassOptions.SpellClassMask[0] & 0x8044C402))
                            continue;
        ...
        switch (spellProto->ClassOptions.SpellClassSet)
        {
            case SPELLFAMILY_ROGUE:
            {
                ...
                if (spellProto->HasAttribute(SPELL_ATTR1_REQ_COMBO_POINTS1) && damagetype != DOT)
            ...
            case SPELLFAMILY_MAGE:
                // Ice Lance
                if (spellProto->Misc.SpellIconID == 186)
                {
                ...
                if (spellProto->GetSchoolMask() & SPELL_SCHOOL_MASK_ARCANE)
                {
                    if (victim->HasAuraWithMechanic((1<<MECHANIC_SNARE)|(1<<MECHANIC_COMBAT_SLOW)))
                    {
                        ...
                        {
                            if ((*i)->GetSpellInfo()->Misc.SpellIconID == 2215)
                            {
                                AddPct(DoneTotalMod, (*i)->GetAmount());
                                break;
                            }
            ...
            {
                // Fire and Brimstone
                if (spellProto->ClassOptions.SpellClassMask[1] & 0x00020040)
                    if (victim->HasAuraState(AURA_STATE_CONFLAGRATE))
                    {
                        AuraEffectList const& mDumyAuras = GetAuraEffectsByType(SPELL_AURA_DUMMY);
                        for (AuraEffectList::const_iterator i = mDumyAuras.begin(); i != mDumyAuras.end(); ++i)
                            if ((*i)->GetSpellInfo()->Misc.SpellIconID == 3173)
                            {
                                AddPct(DoneTotalMod, (*i)->GetAmount());
                                break;
                            }
            ...
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                if (spellProto->ClassOptions.SpellClassMask[0] & 0x2000)
        ...
        if (HasUnitTypeMask(UNIT_MASK_GUARDIAN) && spellProto->Misc.SchoolMask & SPELL_SCHOOL_MASK_MAGIC)
        ...
        if (getPowerType() == POWER_MANA)
        {
            ...
            if (!doneFromManaPctAuras.empty())
            {
                ...
                {
                    if (spellProto->Misc.SchoolMask & (*itr)->GetMiscValue())
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (victim)
            {
                ...
                switch (spellProto->ClassOptions.SpellClassSet)
                {
                    case SPELLFAMILY_MAGE:
                        // Glyph of Fire Blast
                        if (spellProto->ClassOptions.SpellClassMask[0] == 0x2 && spellProto->Misc.SpellIconID == 12)
                    ...
                    case SPELLFAMILY_DRUID:
                        ...
                        // Starfire
                        if (spellProto->ClassOptions.SpellClassMask[0] & 0x4 && spellProto->Misc.SpellIconID == 1485)
                        {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            if (victim)
            {
                ...
                switch (spellProto->ClassOptions.SpellClassSet)
                {
                    ...
                    {
                        // Victory Rush
                        if (spellProto->ClassOptions.SpellClassMask[1] & 0x100)
                        {
    ...
    switch (getClass())
    {
        case CLASS_SHAMAN:
        {
            if (Aura* aura = GetAura(118473))
                if (AuraEffect* eff = aura->GetEffect(EFFECT_0))
                {
                    if (spellProto->Cooldowns.StartRecoveryTime)
                    {
...
void Unit::UpdateMount() //@TODO:Legion - is this really needed?
{
    ...
    for (AuraEffectList::const_reverse_iterator itr = auras.rbegin(); itr != auras.rend(); ++itr)
    {
        ...
        if (MountEntry const* mountEntry = sDB2Manager.GetMount(aura->GetId()))
            mountType = sMountTypeStore.LookupEntry(mountEntry->ID);
        if (mountType)
        {
            effect = aura;
            break;
    ...
    if (mountType)
    {
        DB2Manager::MountTypeXCapabilitySet const* capabilities = sDB2Manager.GetMountCapabilities(mountType->ID);
        if (!capabilities)
            return;
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
    {
        ...
        if (HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE))
        {
            ...
            if (!(bySpell->HasAttribute(SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS)))
...
int32 Unit::ModSpellDuration(SpellInfo const* spellProto, Unit const* target, int32 duration, bool positive, uint32 effectMask, Unit* caster)
{
    ...
    if (target == this)
    {
        ...
        {
            case SPELLFAMILY_DRUID:
                if (spellProto->ClassOptions.SpellClassMask[0] & 0x100)
                {
                    // Glyph of Thorns
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell)
{
    ...
    if (!(spellProto->Misc.Attributes[0] & (SPELL_ATTR0_ABILITY|SPELL_ATTR0_TRADESPELL)) && ((GetTypeId() == TYPEID_PLAYER && spellProto->ClassOptions.SpellClassSet) || GetTypeId() == TYPEID_UNIT))
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, DamageInfo* dmgInfoProc, SpellInfo const* procAura, std::list<uint32>* mSpellModsList)
{
    ...
    HealInfo healInfo = HealInfo(actor, actionTarget, dmgInfoProc->GetDamage(), procSpell, procSpell ? SpellSchoolMask(procSpell->Misc.SchoolMask) : SPELL_SCHOOL_MASK_NORMAL);
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        if(spellInfo->AuraOptions.ProcTypeMask & (PROC_FLAG_DONE_SPELL_MAGIC_DMG_POS_NEG))
            useCharges = true;

        AuraApplication const* aurApp = i->aura->GetApplicationOfTarget(GetGUID());

        // For players set spell cooldown if need
        double cooldown = spellInfo->AuraOptions.ProcCategoryRecovery / 1000.0;
            ...
            for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                switch (triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_PROC_TRIGGER_DAMAGE:
                    {
                        ...
                        SpellNonMeleeDamage damageInfo(this, target, spellInfo->Id, spellInfo->Misc.SchoolMask);
                    ...
                    case SPELL_AURA_REFLECT_SPELLS_SCHOOL:
                        ...
                        if (procSpell && (triggeredByAura->GetMiscValue() & procSpell->Misc.SchoolMask))         // School check
                            takeCharges = true;
                        break;
                        ...
                        switch (triggeredByAura->GetId())
                        {
                            ...
                            case 2094: // Blind
                            {
                                if (procExtra & PROC_EX_INTERNAL_DOT)
                                    if (procSpell->ClassOptions.SpellClassSet == SPELLFAMILY_ROGUE)
...
bool Unit::SpellProcTriggered(Unit* victim, DamageInfo* dmgInfoProc, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, double cooldown)
{
    ...
    Unit* procSpellCaster = dmgInfoProc->GetAttacker();
    ObjectGuid procSpellCasterGUID = procSpellCaster ? procSpellCaster->GetGUID() : ObjectGuid::Empty;
    int32 schoolMask = procSpell ? SpellSchoolMask(procSpell->Misc.SchoolMask) : SPELL_SCHOOL_MASK_NORMAL;
        ...
        for (std::vector<SpellTriggered>::const_iterator itr = spellTrigger->begin(); itr != spellTrigger->end(); ++itr)
        {
            ...
            switch (itr->option)
            {
                ...
                case SPELL_TRIGGER_CAST_AFTER_MAX_STACK: // 37
                {
                    ...
                    {
                        if ((aura->GetStackAmount() + 1) > (aura->GetSpellInfo()->AuraOptions.CumulativeAura - 1))
                        {
    ...
    if (std::vector<SpellPrcoCheck> const* spellCheck = sSpellMgr->GetSpellPrcoCheck(spellProto->Id))
    {
        for (std::vector<SpellPrcoCheck>::const_iterator itr = spellCheck->begin(); itr != spellCheck->end(); ++itr)
        {
            ...
            else if (itr->checkspell == 0)
            {
                ...
                if(procSpell && itr->spellAttr0 > 0 && !(procSpell->Misc.Attributes[0] & itr->spellAttr0))
                {
                    procCheckSecond = true;
                    continue;
                }
                else if(procSpell && itr->spellAttr0 < 0 && (procSpell->Misc.Attributes[0] & abs(itr->spellAttr0)))
                {
                    procCheckSecond = true;
                    continue;
                }
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, SpellInfo const* spellProto, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent, uint8 effect)
{
    ...
    if (spellProcEvent && spellProcEvent->procFlags) // if exist get custom spellProcEvent->procFlags
        ...
        EventProcFlag = spellProto->AuraOptions.ProcTypeMask;       // else get from spell proto
    ...
    if (procExtra & PROC_EX_INTERNAL_TRIGGERED && !(procFlag & PROC_FLAG_DONE_TRAP_ACTIVATION))
    {
        if (!(spellProto->HasAttribute(SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED)))
            return false;
        else if(spellProto->AuraOptions.ProcTypeMask & (PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_NEG|PROC_FLAG_DONE_SPELL_NONE_DMG_CLASS_POS))// for triggered spell set to active
            active = true;
    }
    ...
    if (procSpell && procSpell->Id == spellProto->Id && !(spellProto->AuraOptions.ProcTypeMask & (PROC_FLAG_KILL)))
        return false;
...
void Unit::Kill(Unit* victim, bool durabilityLoss, SpellInfo const* spellProto)
{
    ...
    if (isPet() || isTotem())
        if (Unit* owner = GetOwner())
        {
            DamageInfo dmgInfoProc = DamageInfo(owner, victim, 0, spellProto ? spellProto : NULL, spellProto ? SpellSchoolMask(spellProto->Misc.SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE, 0);
            owner->ProcDamageAndSpell(victim, PROC_FLAG_KILL, PROC_FLAG_NONE, PROC_EX_NONE, &dmgInfoProc);
        }

    if (victim->GetCreatureType() != CREATURE_TYPE_CRITTER)
    {
        DamageInfo dmgInfoProc = DamageInfo(this, victim, 0, spellProto ? spellProto : NULL, spellProto ? SpellSchoolMask(spellProto->Misc.SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, SPELL_DIRECT_DAMAGE, 0);
...
void Unit::RemoveSoulSwapDOT(Unit* target)
{
    ...
    for (AuraEffectList::const_iterator iter = mPeriodic.begin(); iter != mPeriodic.end(); ++iter)
    {
        ...
        if ((*iter)->GetSpellInfo()->ClassOptions.SpellClassSet != SPELLFAMILY_WARLOCK ||
...
void Unit::SendSpellCreateVisual(SpellInfo const* spellInfo, Position const* position, Unit* target, uint32 type, uint32 visualId)
{
    ...
    visual.TravelSpeed = spellInfo->Misc.Speed;
...
void Unit::SendFakeAuraUpdate(uint32 auraId, uint32 flags, uint32 duration, uint32 _slot, bool remove)
{
    ...
    if (!remove)
    {
        ...
        auraData.CastGuid = ObjectGuid::Create<HighGuid::Cast>(sObjectMgr->GetGenerator<HighGuid::Cast>()->Generate());
