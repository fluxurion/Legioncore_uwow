    ...
    */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
    {
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            sWorld->AddCharacterNameData(delInfo.Guid.GetGUIDLow(), delInfo.name, (*result)[2].GetUInt32() & 0xFF, (*result)[0].GetUInt8(), (*result)[1].GetUInt8(), (*result)[2].GetUInt8());
    }
    ...
    */
    ...
    static bool HandlePDumpLoadCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (nameStr)
        {
            ...
            if (ObjectMgr::CheckPlayerName(name, handler->GetSessionDbcLocale(), true) != CHAR_NAME_SUCCESS)
            {
                handler->PSendSysMessage(LANG_INVALID_CHARACTER_NAME);
                handler->SetSentErrorMessage(true);
                return false;
